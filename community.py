...
def CNM(G):
    ...
    for node in G:
        ...
        H[(node,)] = max(dqr.items(), key=itemgetter(1))
    ...
    while len(deltaQ) > 1:
        ...
        (i, (j,mdq)) = max(H.items(), key=embeddedkey)
        ...
        for key in marker.keys():
            ...
            if col == j or col == i:
                H[key] = max(deltaQ[key].items(), key=itemgetter(1))
        ...
        deltaQ[label] = cij
        try:
            H[label] = max(cij.items(), key=itemgetter(1))
        except ValueError:
            pass
        a[label] = ai + aj
    ...
    return (maxQ,maxCommunities,tree,list(deltaQ.keys())[0])